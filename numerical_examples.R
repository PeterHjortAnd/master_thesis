...
# Extract every mn-th point, including the starting point 0
...
lambda_opt <- optimize(obj_funct, interval = c(0.001,300000))$minimum
...
for (j in 1:simtim){
  ...
  sum_terms <- cumsum((sigma_vol[2:(number_of_points+1)])^2)
  result <- sum_terms[indices]
  IV_scale <- c(0, 2^-capital_N * result)
...
#P-th variation roughness estimate
X <- sigma_vol_win
X <- daily_vol
objective_function <- function(p) {
...
S_or <- numeric(number_of_points+1)
S_or[1] <- S0
for (i in 1:number_of_points){
...
plot(RV_scale, type = "l", col = "black", lwd = 2, 
     xlab = "Time",xaxt = "n", ylab = "Volatility",  main = "")
lines(IV_scale, col = "red", lwd = 2)
IV_minus_RV <- IV_scale-RV_scale
...
# Density plot for remaining estimators
simtim <- 10
H_IV_log <- numeric(simtim)
H_RV_log <- numeric(simtim)
H_IV_scale <- numeric(simtim)
H_RV_scale <- numeric(simtim)
H_IV_expo <- numeric(simtim)
H_RV_expo <- numeric(simtim)
...
for (j in 1:simtim){
  ...
  H_IV_log[j] <- smoothness_fct(IV)
  H_RV_log[j] <- smoothness_fct(RV)
  
  #Rescale Brownian motions
  dB1_new <- sqrt_dt_new*1/sqrt_dt*dB_1[1:number_of_points]
  dB2_new <- sqrt_dt_new*1/sqrt_dt*dB_2[1:number_of_points]
  ...
  S[1] <- S0
  for (i in 1:number_of_points){
    S[i+1] <- S[i]+S[i]*sigma[i]*dB1_new[i]
  }
  ...
  sum_terms <- cumsum(exp(2*Y[2:(number_of_points+1)]))
  result <- sum_terms[indices]
  IV_scale <- c(0, 2^-capital_N * result)
...
plot(density(H_IV_log), 
     main = "",
     xlab = "Estimated roughness",
     ylab = "Density",
     xlim = c(min(c(H_IV_log, H_IV_scale, H_IV_expo)),max(c(H_IV_log,H_IV_scale, H_IV_expo))),
     col = "red", 
     lwd = 2)  # Density plot of roughness estimate
lines(density(H_IV_scale), col = 'blue', lwd =2)
lines(density(H_IV_expo), col = 'green', lwd =2)

plot(density(H_RV_log), 
     main = "",
     xlab = "Estimated roughness",
     ylab = "Density",
     xlim = c(min(c(H_RV_log,H_RV_scale, H_RV_expo)),max(c(H_RV_log,H_RV_scale, H_RV_expo))),
     col = "red", 
     lwd = 2)  # Density plot of roughness estimate
lines(density(H_RV_scale), col = 'blue', lwd =2)
lines(density(H_RV_expo), col = 'green', lwd =2)
...
output_given_H_new <- function(H){
  ...
  # Realized volatility
  log_returns <- diff(log(abs(S)))
  squared_log_returns <- log_returns[1:(window_length*window)]^2
  # Realized volatility calculation (vectorized)
  real_vol <- sqrt(rowSums(matrix(squared_log_returns, nrow = window_length, ncol = window, byrow = TRUE)))
  RV <- real_vol * sqrt_term
  IV <- sigma[seq(1, n, by = window)]
...
store_results <- list()
for (H in H_values) {
  store_results[[paste0("H_", H)]] <- output_given_H_new(H)
}
results <- data.frame(H = numeric(), Instantaneous_volatility = numeric(), Realized_volatility = numeric(), IV_logres = numeric(), RV_logres = numeric(),
...
simoutput_given_H <- function(H){
  ...
  return(list(IV_H = H_IV_scale, RV_H = H_RV_scale))
}
sim_vol_table <- function(H_values){
  volatility_table <- data.frame(
    'Instantaneous volatility' = numeric(0),
...
abline(v = seq(0, 0.8, by = 0.1), col = "gray", lty = "dotted", lwd = 0.5) # Vertical lines
abline(h = seq(0, 0.8, by = 0.1), col = "gray", lty = "dotted", lwd = 0.5) # Horizontal lines
legend("topleft", legend = c('Realized Var', 'Integrated Var'), col = c('red', 'blue'), lty = 1, cex = 0.7)
